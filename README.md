...
> The `State` column contains Canadian province names (legacy column name from the original Superstore dataset).
...
|----------|------------------|---------|
| `GET /api/meta` | - | All unique filter values |
...
| `GET /api/category` | filters | Sales + profit grouped by category |
...
4. Set `DATA_PATH` env variable pointing to your CSV
...
- Market basket analysis is slow on large unfiltered datasets
...
- Natural language insights via LLM
